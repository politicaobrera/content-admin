...
**Image handling.** Images are uploaded to Firebase Storage (`app/services/firebase/storage.ts`) and compressed client-side with `browser-image-compression` before upload; see `app/hooks/image/useMainImage.ts` and `useImageResource.ts`.
